...
data pred; 
	merge pred (in = a) xold.hurdprobabilities_wide;
	by hhid pn;
	if a;
run;
...
proc freq data=pred; tables exclude_b exclude_d; run;

/*create common sample flag
2 versions:
	- commsamp: non-missing for all algorithmic classifications
	- commsamp2: non-missing for all algorithmic classifications for all self-response and non-dementia classified proxy respondents 
		- Crimmins proxy algorithm excluded HRS proxy respondents calssifed as "normal"
*/
/*%macro com(w);*/
/*data pred; set pred;*/
/*	if hw_dem_&w NE . and lkw_dem_&w NE . and crim_dem11_&w NE .  and hurd_dem_&w NE . and wu_dem_&w NE . then commsamp_&w = 1;*/
/*	proc freq; tables commsamp_&w ;*/
/*run;*/
/*%mend;*/
/*%com(A) %com(B) %com(C)%com(D)*/

/*MERGE IN COMMON SAMPLE FLAG that accounts for missing Hurd algorithm predictors (despite having author-provided probabilities*/
data pred;
	merge pred x.commsampfinal_key;
	by hhid pn;
run;
